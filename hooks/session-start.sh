...
if [[ "$("${CLAUDE_PLUGIN_ROOT}/scripts/settings.sh" workflow 2>/dev/null)" == build ]]; then
  context="This project uses the sdlc workflow for new work: when the user asks to build, create or change something, start with the sdlc:build skill (design in plan mode, then split and dispatch), unless the user asks to do it another way."
else
  exit 0
fi
