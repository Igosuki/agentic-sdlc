...
if [[ -n "${DISPATCH_TASK:-}" ]]; then
  ...
  finish="${CLAUDE_PLUGIN_ROOT}/skills/dispatch/scripts/finish-task.sh"
...
elif [[ "$("${CLAUDE_PLUGIN_ROOT}/skills/dispatch/scripts/settings.sh" workflow 2>/dev/null)" == build ]]; then
