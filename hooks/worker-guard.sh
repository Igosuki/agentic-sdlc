...
[[ ! "$cmd" =~ ${start}git([[:space:]]+-[^[:space:]]+([[:space:]]+[^[:space:]-][^[:space:]]*)?)*[[:space:]]+push([[:space:]]|$) ]] \
  || deny "Workers don't push. Run $finish: it merges, or pushes and opens the PR in epic-pr mode."
[[ ! "$cmd" =~ ${start}gh[[:space:]]+pr[[:space:]]+merge([[:space:]]|$) ]] \
  || deny "Workers don't merge PRs. Run $finish: it merges through the merge queue."
