...
worktree and branch with wt remove -D (if one exists), and reopens the task
with every dispatch_* metadata key unset. A later dispatch picks the task up
like new. dispatch, split and /sdlc:recover all call this instead of writing
