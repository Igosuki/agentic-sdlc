...
Watch with the Monitor tool, with `persistent: true` so the watch doesn't time out while workers run, and command `${CLAUDE_PLUGIN_ROOT}/scripts/watch.py`, plus `--epic <id>` when there is one. The watch takes no `--parallel`. A headless session keeps running while the watch lasts. Each line is an event:
